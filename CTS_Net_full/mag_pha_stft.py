...
class mag_pha_STFT(torch.nn.Module):
    def __init__(self, filter_length=320, hop_length=160):
        super(mag_pha_STFT, self).__init__()
    ...
    def transform(self, input_data):
        ...
        forward_transform = F.conv1d(input_data,
                                     Variable(self.forward_basis, requires_grad=False).to(input_data.device),
                                     stride=self.hop_length,
                                     padding=0)
        cutoff = int((self.filter_length / 2) + 1)
        real_part = forward_transform[:, :cutoff, :]
        imag_part = forward_transform[:, cutoff:, :]
        real_part, imag_part = real_part.permute(0, 2, 1).contiguous(), imag_part.permute(0, 2, 1).contiguous()
        magnitude = torch.sqrt(real_part ** 2 + imag_part ** 2)
        phase = torch.atan2(imag_part, real_part)
        return [magnitude, phase]

    # list, magnitude: (B, T, F), phase: (B, T, F)
    def inverse(self, stft_res):
        [magnitude, phase] = stft_res
        real_part = (magnitude * torch.cos(phase)).permute(0, 2, 1)
        imag_part = (magnitude * torch.sin(phase)).permute(0, 2, 1)
        ...
        inverse_transform = F.conv_transpose1d(recombine_magnitude_phase,
                                               Variable(self.inverse_basis, requires_grad=False).to(magnitude.device),
                                               stride=self.hop_length,
                                               padding=0)
...
class com_STFT(torch.nn.Module):
    def __init__(self, filter_length=320, hop_length=160):
        super(com_STFT, self).__init__()
    ...
    def inverse(self, stft_res):
        ...
        recombine_magnitude_phase = torch.cat([real_part, imag_part], dim=1)
        inverse_transform = F.conv_transpose1d(recombine_magnitude_phase,
                                               Variable(self.inverse_basis, requires_grad=False).to(stft_res.device),
                                               stride=self.hop_length,
                                               padding=0)
        return inverse_transform[:, 0, :]
