...
import time
import os
from Backup import com_mse_loss, com_mag_mse_loss, pesq_loss
import hdf5storage
import gc
from config_gcrn import *
    ...
    def run_one_epoch(self, epoch, cross_valid=False):
        def _batch(_, batch_info):
            with set_default_tensor_type(torch.cuda.FloatTensor):
                ...
                esti = self.model(batch_feat)

                if not cross_valid:
                    batch_loss = com_mag_mse_loss(esti, batch_label, batch_frame_mask_list)
                    ...
                    if is_pesq:
                        batch_loss_res = pesq_loss(esti, batch_label, batch_frame_mask_list)
                    else:
                        batch_loss = com_mag_mse_loss(esti, batch_label, batch_frame_mask_list)
                        batch_loss_res = batch_loss.item()
                    tr_batch.append(batch_loss_res)
