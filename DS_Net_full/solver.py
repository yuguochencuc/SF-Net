...
import time
import os
from Backup import com_mse_loss, com_mag_mse_loss, pesq_loss, mag_mse_loss
import hdf5storage
import gc
...
class Solver(object):
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if self.is_cp:
                cp_dic = {}
                cp_dic['model_state_dict'] = self.model.state_dict()
                ...
                torch.save(cp_dic, os.path.join(self.cp_path, 'denoise_checkpoint_early_exit_%dth.pth.tar' % (epoch + 1)))
    ...
    def run_one_epoch(self, epoch, cross_valid=False):
        def _batch(_, batch_info):
            ...
            if not cross_valid:
                batch_loss = mag_mse_loss(esti_list, batch_label, batch_frame_mask_list)
