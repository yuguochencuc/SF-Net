...
class CP_denoise_net_tcm1(nn.Module):
    def __init__(self, X, R, is_gate=True):
        super(CP_denoise_net_tcm1, self).__init__()
        self.X, self.R = X, R
        self.is_gate = is_gate
        self.en_ri = Encoder(ci=2, is_gate=self.is_gate)
        self.de_r = Decoder(is_gate=self.is_gate)
    ...
    def forward(self, x):
        ...
        x_com_out = torch.stack((x_r,x_i),dim=1)

        return x_com_out

class ME_denoise_net_tcm1(nn.Module):
    def __init__(self, X, R, is_gate=True):
        super(ME_denoise_net_tcm1, self).__init__()
        self.X, self.R = X, R
        self.is_gate = is_gate
        self.en_mag = Encoder(ci=1, is_gate=self.is_gate)
        self.de_mask = Decoder_mask(is_gate=self.is_gate)
...
class Encoder(nn.Module):
    def __init__(self, ci, is_gate):
        ...
        en6 = Conv_block_last(64, 64, (2, 3), self.is_gate)
        en7 = Conv_block_last(64, 64, (2, 3), self.is_gate)
...
class Decoder_mask(nn.Module):
    def __init__(self,  is_gate):
        ...
        self.is_gate = is_gate
        de1 = Deconv_block(64, 64, (2, 1), is_gate=self.is_gate)
        de2 = Deconv_block(64, 64, (2, 2), is_gate=self.is_gate)
        de3 = Deconv_block(64, 64, (2, 3), is_gate=self.is_gate)
        de4 = Deconv_block(64, 64, (2, 3), is_gate=self.is_gate)
        de5 = Deconv_block(64, 64, (2, 3), is_gate=self.is_gate)
        de6 = Deconv_block(64, 64, (2, 3), is_gate=self.is_gate)
        de7 = Deconv_block(64, 1, (2, 5), is_gate=self.is_gate)
...
class Conv_block_last(nn.Module):
    def __init__(self, ci, co, k, is_gate):
        ...
        conv_list = []
        if self.is_gate is True:
            conv_list.append(Gate_Conv(self.ci, self.co, self.k, stride=(1, 2), de_flag=0, pad=(1, 1, 1, 0)))
        else:
            conv_list.append(nn.ConstantPad2d((1, 1, 1, 0), value=0.))
            conv_list.append(nn.Conv2d(self.ci, self.co, kernel_size=self.k, stride=(1, 2)))
        conv_list.append(NormSwitch('cLN', '2D', self.co))
        conv_list.append(nn.PReLU(self.co))
        self.convs = nn.Sequential(*conv_list)
